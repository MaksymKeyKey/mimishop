...
const ItemProductForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`http://13.53.147.216http://13.53.147.216/api/itemProducts${isEditing ? `/${itemProduct._id}` : ''}`, {
                method,
...
const FilterForm = () => {
    ...
    const handleDelete = async (_id) => {
        try {
            const response = await fetch(`http://localhost:5000http://13.53.147.216/api/filters/${_id}`, { method: 'DELETE' });
